...
- **[ESP32 Bus Pirate](firmware/buspirate/)** — a multi-protocol hardware-hacking multitool on the badge TFT (also USB-serial / Wi-Fi web CLI): I²C/SPI/UART/1-Wire/JTAG bus tools with **on-screen bus scan + live register monitor** and a **BME280 dashboard** that colours the ears by temperature, plus a full **on-screen LoRa/RF suite** — heat-map **waterfall**, packet **sniffer**, live **RSSI meter**, band **scanner**, and a **Meshtastic monitor** that decodes real mesh packets on-screen (with a **MeshCore** sniffer preset). Wi-Fi/BLE scanners too. **Start with the [beginner's guide](firmware/buspirate/GUIDE.md).** ⚠ Transmit- & attack-capable (Wi-Fi deauth, BLE spoof, RF jam) — **authorized use only**. Ready-to-flash: **[RetiaLLC/ESP32-Bus-Pirate @ badge-v1.2.1](https://github.com/RetiaLLC/ESP32-Bus-Pirate/releases/tag/badge-v1.2.1)**, ported from [KonradIT/geo-tp](https://github.com/KonradIT/ESP32-Bus-Pirate).
