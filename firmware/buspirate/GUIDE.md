...
- **Over Wi-Fi (no cable) — the web CLI is a *boot mode*, not a serial command.**
  **Reset the badge** — a terminal-type menu appears for ~6 s: **d-pad LEFT/RIGHT** to
  ...
  > The interactive terminal works from any browser, phones included — type a command and
  > the output streams back live. (The earlier "Connection lost" flakiness is fixed.) Web
  > Serial over USB (above) is still there if you'd rather stay on a cable.
