...
| `meshtastic-2.7.23-mui-touch.factory.bin` | **Experimental color touch UI** (Meshtastic device-ui / LVGL). Full touchscreen operation — on-screen keyboard, tap navigation — despite the badge's crossed touch data lines (worked around with a calibrated bit-banged XPT2046 driver). D-pad + A/B also work as a keypad. Known limitation: while BLE is active the screen shows the pairing view instead of home. |
