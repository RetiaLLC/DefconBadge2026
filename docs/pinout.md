...
9. **⚠️ Qwiic/STEMMA-QT connectors (J4, J6) are pin-reversed vs. the standard — do NOT plug in stock Qwiic sensors.** The JST-SH footprint is mounted so the physical pin order is **backwards** relative to the SparkFun-Qwiic / Adafruit-STEMMA-QT cable standard, so a standard cable lands **power and ground swapped/scrambled** — the sensor never powers up and a small host (e.g. a Nugget) can brown out and fail to boot. Affects both I²C (J6) and the UART-on-Qwiic (J4), and the same footprint is shared across other Retia boards. **Workaround:** wire I²C to the **SAO header (J5)** instead — pin 1 = +3.3 V, 2 = GND, 3 = SDA/GPIO35, 4 = SCL/GPIO36 — after metering. Full analysis + fix in [hardware-errata.md](hardware-errata.md).
